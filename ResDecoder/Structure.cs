...
namespace ResDecoder
{
    ...
    public class TbClassSpeech : ResReader
    {
        ...
        {
            i = new uint[2 + Envirment.NumberOfCharacter * 6];
        }

        override public void Read(FileStream fs)
        {
            int k = 1;
            for (int j = 1; j <= Envirment.NumberOfCharacter + 1; j++)
                i[k++] = GetUint(fs);
            for (int j = 1; j <= Envirment.NumberOfCharacter*2; j++)
                i[k++] = GetByte(fs);
            for (int j = 1; j <= Envirment.NumberOfCharacter; j++)
                i[k++] = GetUint(fs);
            for (int j = 1; j <= Envirment.NumberOfCharacter; j++)
                i[k++] = GetByte(fs);
            for (int j = 1; j <= Envirment.NumberOfCharacter; j++)
        ...
        override public void Write(StringBuilder sb)
        {
            ...
            for (int j = 2; j <= 1 + Envirment.NumberOfCharacter*6; j++)
                sb.AppendLine($"{i[j]}");
    ...
    public class TbSpeechTag : ResReader
    {
        ...
        public TbSpeechTag()
        {
            i = new uint[Envirment.NumberOfCharacter];
        }
        ...
        override public void Read(FileStream fs)
        {
            ...
            str1 = GetString(fs);
            b2 = GetByte(fs);
            for (int j = 0; j < Envirment.NumberOfCharacter; j++)
                i[j] = GetUint(fs);
        }
        ...
        override public void Write(StringBuilder sb)
        {
            ...
            for (int j = 0, k = 2; j < Envirment.NumberOfCharacter; j++, k++)
                sb.AppendLine($"NUM{k}={i[j]}");
